...
//        };
        class future_waiter
        {
            ...
            struct all_futures_lock
            {
                ...
                all_futures_lock(std::vector<registered_waiter>& inFutures):
                    count(inFutures.size()),locks(new boost::unique_lock<boost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
#if defined __DECCXX || defined __SUNPRO_CC || defined __hpux
                        locks[i]=boost::unique_lock<boost::mutex>(inFutures[i].future_->mutex).move();
#else
                        locks[i]=boost::unique_lock<boost::mutex>(inFutures[i].future_->mutex);
#endif
                    }
                }
